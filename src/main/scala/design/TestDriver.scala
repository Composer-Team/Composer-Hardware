...
object Driver extends App {
  ...
  new RocketChipStage().execute(Array("-td", hw_idr, "-T", "composer.ComposerTop", "-C", "design.GemmConfig"), Seq())
//  run(Seq(
...
//    new ConfigsAnnotation(Seq("design.exampleConfig"))))
  FirrtlMain.stage.execute(Array("-i", hw_idr + "/composer.GemmConfig.fir", "-o", hw_idr + "composer.v", "-X", "verilog"), Seq())
}