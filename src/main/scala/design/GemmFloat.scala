...
import composer.common.{DecoupledFloat2ALU, Float2ALUOp}
...
/**
  ...
  */
case class GemmFloatParam(rowColDim: Int,
                     columnParallelism: Int,
                     rowParallelism: Int,
                     maxRCDim: Int)

class GemmFloatCore(composerCoreParams: ComposerConstructor, coreP: GemmFloatParam)(implicit p: Parameters) extends ComposerCore(composerCoreParams)(p) {
  val dataWidthBytes = 4
  ...
  val BCache = Seq.fill(arithUnits)(SyncReadMem(elemsPerArithUnit, UInt(dataWidthBits.W)))
  ...
  val OCache = Seq.fill(coreP.rowParallelism, arithUnits)(SyncReadMem(elemsPerArithUnit, UInt(dataWidthBits.W)))
  val OAccs = Seq.fill(coreP.rowParallelism, arithUnits)(Reg(UInt(dataWidthBits.W)))
  // 🍞🥖
  ...
  OCache.zip(OAccs).foreach { case (ocache_row: Seq[SyncReadMem[UInt]], oacc_row: Seq[UInt]) =>
    ocache_row zip oacc_row map { case (oc: SyncReadMem[UInt], oa: UInt) =>
      val do_read = (state === s_acc) || (state === s_writeback_1)
      val cache_read = Wire(UInt(dataWidthBits.W))
...
//      }
  ...
  // after 1 cy
  val bread_val = Seq.fill(arithUnits)(Wire(UInt(dataWidthBits.W)))
  ...
  val fpu_latency = 3
  // avail after 2 cy
  val f_res = current_a.zip(OAccs).flatMap { case (row_a, acc_col) =>
    bread_val.zip(acc_col) map { case (arith_b, acc_ele) =>
      // do this to over-ride inferred width
      val fpu = Module(new FPUFMAPipe(latency = fpu_latency, t = FType.S))
      fpu.io.in.bits.in1 := row_a
      fpu.io.in.bits.in2 := arith_b
      fpu.io.in.bits.in3 := acc_ele
      fpu.io.in.valid := RegNext(state === s_acc)

      // https://riscv.org/wp-content/uploads/2017/05/riscv-spec-v2.2.pdf
      // page 48 onwards details flags for fpus
      fpu.io.in.bits.rm := 0.U // round to nearest
      fpu.io.in.bits.fmt := 0.U // Single precision
      // page 107 in manual shows opcode
      // line 830 in rocketchip/tile/FPU.scala shows how to decode fmaCmd
      fpu.io.in.bits.fmaCmd := 0.U
      fpu.io.in.bits.typ := 0.U // it's Single precision

      // control flags
      fpu.io.in.bits.div := false.B
      fpu.io.in.bits.wen := true.B
      fpu.io.in.bits.ren1 := true.B
      fpu.io.in.bits.ren2 := true.B
      fpu.io.in.bits.ren3 := true.B
      fpu.io.in.bits.swap12 := false.B
      fpu.io.in.bits.swap23 := false.B
      fpu.io.in.bits.typeTagIn := true.B // Using S
      fpu.io.in.bits.typeTagOut := true.B // using S
      fpu.io.in.bits.fromint := false.B
      fpu.io.in.bits.toint := false.B
      fpu.io.in.bits.fma := true.B
      fpu.io.in.bits.wflags := true.B
      fpu.io.in.bits.fastpipe := false.B
      fpu.io.in.bits.ldst := false.B
      fpu.io.out.bits.data
    }
  }


  val addr = RegNext(RegNext(bread))
  val do_write = Pipe(true.B, state === s_acc, fpu_latency + 1)
  f_res zip OCache.flatten foreach { case (o_acc_dat, ocache) =>
    when(do_write.bits) {
        ...
        dataChannelRow map (_.data.bits) zip OCache foreach { case (dat, cache) =>
          cache.zipWithIndex foreach { case (arithcache, idx) =>
            when(idx.U === arithCounter) {
              arithcache.write(bread, dat.asUInt)
            }
          }
        }
      }
    ...
    is(s_load) {
      ...
      when(dataChannelB.data.fire) {
        ...
        BCache zip splitPayload(dataChannelB.data.bits, dataWidthBits) foreach { case (cache, dat) =>
          cache.write(b_loadidx, dat.asUInt)
        }
...
class WithGemmFloat(withNCores: Int,
               gp: GemmFloatParam) extends Config((site, _, up) => {
