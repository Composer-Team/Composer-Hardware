...
import composer.Generation.{ComposerBuild, ComposerChipStage}
import composer.Generation.Stage.ComposerAnnotations
import composer.Systems.AcceleratorCore
...
object LazyFloorplan {
  def registerSystem[T <: AcceleratorCore](nCores: Int, module: => T, outwardsConnectivity: Iterable[String])(implicit p: Parameters): Unit = {
    ...
    new ChiselStage().emitVerilog(module, annotations = ComposerAnnotations.no_class(new Config(p),
    ...
    os.write.over(fpath, "")
    ComposerBuild.sourceList.foreach {
      path =>
