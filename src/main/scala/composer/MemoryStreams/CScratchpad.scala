...
class CScratchpadInitReqIO(mem_out: TLBundle, nDatas: Int, memLenBits: Int) extends Bundle {
...
class CScratchpadImp(supportWriteback: Boolean,
  ...
  val memoryLengthBits = {
    val nBeats = (nDatas.toFloat / loader.spEntriesPerBeat).ceil.toInt
    val l = nBeats * outer.channelWidthBytes
    log2Up(l) + 1
  }
  ...
  val req = IO(new CScratchpadInitReqIO(mem_out, nDatas, memoryLengthBits))
  ...
  memory.io.r_mem_en := access.readReq.valid
  ...
  access.readRes.bits := memory.io.r_dout
  ...
  when(loader.io.sp_write_out.valid) {
    memory.io.w_mem_en := true.B
    memory.io.w_din := loader.io.sp_write_out.bits.dat
    memory.io.w_addr := loader.io.sp_write_out.bits.idx
  ...
  if (supportWriteback) {
    ...
    channel.data.bits := memory.io.r_dout
    switch(wb_state) {
      is(wb_idle) {
      ...
      is(wb_read) {
        req.request.ready := false.B
        memory.io.r_mem_en := true.B
        memory.io.r_addr := writebackIdx
