...
object CppGeneration {
  ...
  private[composer] case class CppDefinition(ty: String, name: String, value: String)
  ...
  private[composer] var user_enums: List[chisel3.ChiselEnum] = List()
  private[composer] var user_defs: List[CppDefinition] = List()
  private[composer] var user_cpp_defs: List[PreprocessorDefinition] = List()
  private[composer] var hook_defs: List[HookDef] = List()
