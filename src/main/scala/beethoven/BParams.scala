...
import beethoven.BeethovenConstraintHint.BeethovenConstraintHint
import beethoven.Generation.BuildMode
import beethoven.MemoryStreams._
import beethoven.Platforms.{BuildModeKey, Platform, PlatformKey}
import beethoven.Systems._
import beethoven.common.{AccelCommand, AccelResponse, AccelRoccCommand}
...
case class ModuleBuilder(constructor: (BeethovenSystem, Parameters) => AcceleratorCore) extends ModuleConstructor
...
class WithBeethoven(platform: Platform,
                    constraintHints: List[BeethovenConstraintHint] = List.empty,
                    quiet: Boolean = false,
