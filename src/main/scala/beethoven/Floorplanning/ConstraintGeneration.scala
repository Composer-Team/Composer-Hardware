...
import beethoven.Platforms.{MultiDiePlatform, Platform, PlatformKey, PlatformType}
import beethoven.{BeethovenConstraintHint, ConstraintHintsKey}
import os.Path
...
//    println(moduleName)
  ...
  def canDistributeOverSLRs()(implicit p: Parameters): Boolean =
    p(ConstraintHintsKey).contains(BeethovenConstraintHint.DistributeCoresAcrossSLRs) &&
