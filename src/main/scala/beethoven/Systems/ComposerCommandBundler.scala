...
class BeethovenCommandBundler[T1 <: AccelCommand, T2 <: AccelResponse](bundleIn: T1, bundleOut: T2, outer: BeethovenSystem, nSources: Int, opCode: Int)(implicit p: Parameters) extends Module {
  ...
  val cio = IO(new Bundle() {
    val cmd = Flipped(new BeethovenCoreIO)
