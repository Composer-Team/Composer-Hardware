...
import beethoven.RoccHelpers._
...
class BeethovenSystemImp(val outer: BeethovenSystem)(implicit p: Parameters) extends LazyModuleImpWithSLRs(outer) {
  val sw_io = if (outer.systemParams.canReceiveSoftwareCommands) Some(IO(new BeethovenSystemIO())) else None
        ...
        (slr_id, collapseResp(core_list.map {
          case ((c: AcceleratorCore, cn: String), slr_id: Int, core_idx: Int) =>
            val respEPs = c.io_declaration.resp.map { ioresp =>
              val beethovenRespWire = Wire(new AccelRoccResponse())
              beethovenRespWire.getDataField := ioresp.getDataField
              beethovenRespWire.rd := ioresp.rd
              beethovenRespWire.system_id := outer.system_id.U
              beethovenRespWire.core_id := core_idx.U
              beethovenRespWire
            }
            (respEPs, cn)
        }, slr_id))
  ...
  val internalRespDispatchModule = if (outer.canBeIntaCoreCommandEndpoint) {
    ...
    internalRespDispatcher.io.bits.addr := BeethovenConsts.getInternalCmdRoutingAddress(wire.system_id)
  ...
  outer.systemParams.canIssueCoreCommandsTo.foreach { target =>
    ...
    cores.zipWithIndex.foreach { case ((core, _), core_idx) =>
      core.beethoven_response_ios_(target).valid := responseManager.io.valid && response.core_id === core_idx.U
      core.beethoven_response_ios_(target).bits := response
    }

    responseManager.io.ready := VecInit(cores.map(a => a._1.beethoven_response_ios_(target).ready))(response.core_id)
  }
