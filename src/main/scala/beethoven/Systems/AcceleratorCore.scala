...
class BeethovenCoreIO(implicit p: Parameters) extends CustomIO[AccelRoccCommand, AccelRoccUserResponse](new AccelRoccCommand, new AccelRoccUserResponse, -1)
...
class AcceleratorCore(val outer: BeethovenSystem)(implicit p: Parameters) extends Module {
  ...
  val io_declaration = IO(Flipped(new BeethovenCoreIO()))
  ...
  //  def getCoreID: Int = beethovenConstructor.beethovenCoreWrapper.core_id
  ...
  val read_ios = Map.from(outer.memParams.filter(_.isInstanceOf[ReadChannelParams]).map {
    case mp: ReadChannelParams =>
      (mp.name,
  ...
  val write_ios = Map.from(outer.memParams.filter(_.isInstanceOf[WriteChannelParams]).map {
    case mp: WriteChannelParams =>
      (mp.name,
        (mp, (0 until mp.nChannels).map { channelIdx =>
  ...
  val sp_ios = Map.from(outer.memParams.filter(_.isInstanceOf[ScratchpadParams]).map {
    case mp: ScratchpadParams =>
      (mp.name,
        (mp, ({
  ...
  def BeethovenIO[T1 <: AccelCommand, T2 <: AccelResponse](bundleIn: T1, bundleOut: T2): CustomIO[T1, T2] = {
    ...
    beethovenCustomCommandManager.cio.cmd_in_source := io_source
  ...
  def BeethovenIO(): BeethovenCoreIO = {
...
class AcceleratorBlackBoxCore(outer: BeethovenSystem,
  ...
  val readerParams = outer.systemParams.memoryChannelParams.filter(_.isInstanceOf[ReadChannelParams]).map(_.asInstanceOf[ReadChannelParams])
  val writerParams = outer.systemParams.memoryChannelParams.filter(_.isInstanceOf[WriteChannelParams]).map(_.asInstanceOf[WriteChannelParams])
  val spParams = outer.systemParams.memoryChannelParams.filter(_.isInstanceOf[ScratchpadParams]).map(_.asInstanceOf[ScratchpadParams])
  ...
  val wrapperFName = os.Path(BeethovenBuild.beethovenGenDir) / s"${outer.systemParams.name}_chiselLink.v"
  os.write.over(wrapperFName, bbWrapper)
  BeethovenBuild.addSource(wrapperFName)

  val bbFName = os.Path(BeethovenBuild.beethovenGenDir) / s"${outer.systemParams.name}.v"
  os.write.over(bbFName, userBB)
}

class BeethovenSystemIO(implicit p: Parameters) extends Bundle {
