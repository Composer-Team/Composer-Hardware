...
object BeethovenSystem {
}

class BeethovenSystem(val systemParams: AcceleratorSystemConfig,
                     val system_id: Int,
                     val canBeIntaCoreCommandEndpoint: Boolean,
                     val acc: BeethovenAcc)(implicit p: Parameters) extends LazyModuleWithSLRs {
  ...
  val readers = (0 until nCores) map { core_id =>
    memParams.filter(_.isInstanceOf[ReadChannelParams]).map { para =>
      val param: ReadChannelParams = para.asInstanceOf[ReadChannelParams]
  ...
  val writers = (0 until nCores) map { core_id =>
    memParams.filter(_.isInstanceOf[WriteChannelParams]).map { mcp =>
      val para = mcp.asInstanceOf[WriteChannelParams]
      (para,
  ...
  val scratch_mod = Seq.tabulate(nCores)(coreIdx => memParams.filter(_.isInstanceOf[ScratchpadParams]).map(_.asInstanceOf[ScratchpadParams]).map {
    param =>
...
//              CLogger.log(f"\t$x trees to ${slr - 1}")
  ...
  val (internalCommandManager, incomingInternalCommandXbar) = if (canBeIntaCoreCommandEndpoint) {
    ...
    val manager = TLManagerNode(Seq(TLSlavePortParameters.v1(
      managers = Seq(TLSlaveParameters.v2(
        address = Seq(BeethovenConsts.getInternalCmdRoutingAddressSet(system_id)),
  ...
  val incomingInternalResponseHandlers = Map.from(systemParams.canIssueCoreCommandsTo.map { target =>
    val manager = TLManagerNode(
      Seq(TLSlavePortParameters.v1(
        managers = Seq(TLSlaveParameters.v1(
          Seq(BeethovenConsts.getInternalCmdRoutingAddressSet(system_id)),
  ...
  lazy val module = new BeethovenSystemImp(this)
}
