...
import beethoven.Generation.{BuildArgs, CLogger, BeethovenBuild}
...
object Tunable {
  ...
  private[beethoven] var tunables: Seq[Tunable with TaggedTunable] = Seq.empty
  protected[beethoven] var tuneCounter = 0

  private[beethoven] def exportNames(): Unit = {
    val fname = BeethovenBuild.beethovenGenDir + "/tunables.json"
...
private[beethoven] class UnitTunable(x: Int) extends Tunable {
...
private[beethoven] class TunableSum(sum: Seq[Tunable]) extends Tunable {
...
private[beethoven] class TunableScalarProduct(val tunable: Tunable, val scalar: Int) extends Tunable {
...
private[beethoven] class TunableProduct(val tunables: Seq[Tunable]) extends Tunable {
...
private[beethoven] class TunableScalarSum(val tunable: Tunable, val scalar: Int) extends Tunable {
  override def isIdentity: Boolean = tunable.isIdentity
  override val range: (Int, Int) = (tunable.range._1 + scalar, tunable.range._2 + scalar)
  override val default: Int = tunable.default + scalar
  override def getValue: Int = tunable.getValue + scalar
...
private[beethoven] class TunablePow(val tunable: Tunable, val pow: Int) extends Tunable {
  override def isIdentity: Boolean = tunable.isIdentity
...
private[beethoven] class TunableLog2(val tunable: Tunable) extends Tunable {
  override def isIdentity: Boolean = tunable.isIdentity
