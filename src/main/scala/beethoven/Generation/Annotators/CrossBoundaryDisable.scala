...
object CrossBoundaryDisable {
  def apply(disableList: Seq[String], targetDir: Path): Unit = {
    ...
    //    val beethovenTop = os.read(targetDir / "BeethovenTop.v")
    //    val newBeethovenTop = disableList.foldLeft(beethovenTop) { (acc, module) =>
    //      acc.replaceAll(s"module $module", s"(* keep_hierarchy = \"yes\" *) module $module")
    //    }
    //    os.write.over(targetDir / "BeethovenTop.v", newBeethovenTop)
